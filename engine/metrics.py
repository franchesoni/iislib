...
def intersect_and_union(pred_label,
    ...
    pred_label = torch.from_numpy((pred_label)) if type(pred_label) == np.array else pred_label
    ...
    assert {int(e) for e in set(torch.unique(label))}.issubset({0, 1}), '`label` should be binary'
    assert {int(e) for e in set(torch.unique(pred_label))}.issubset({0, 1}), '`pred_label` should be binary'
