def test_training_logic():
    ...
    # model
    model = EarlySMP(smp.Unet, {'encoder_name':'mobilenet_v2', 'encoder_weights':'imagenet'}, in_channels=6)

    for ind, batch in enumerate(iis_dataloader):
        print(ind)
        images, masks, infos = batch['image'], batch['mask'], batch['info']
        output, pc_mask, nc_mask, pcs, ncs = interact(model, batch, interaction_steps=3, clicks_per_step=3)
