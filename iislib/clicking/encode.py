...
def torch_simplest_disk(radius: int) -> torch.Tensor:
    '''Creates a binary matrix containing a disk of the given radius using torch'''
    out_side = radius * 2 + 1
...
def draw_disk_from_single_coords(point: Point, initial_mask: torch.Tensor, radius: int = 5) -> torch.Tensor:
    """Adds disk of given `radius` centered in `point` to `initial_mask`
    `initial_mask` should be a binary ({0, 1}) matrix
    `points` should be a `torch.Tensor` of size (2,) with (row_ind, col_ind)"""
    ...
    out_mask[
    ...
    ] = torch.logical_or(  # add disk in the correct position accounting for border clipping
        out_mask[
            row_ind - radius + top_offset : row_ind + radius + 1 - bottom_offset,
            col_ind - radius + left_offset : col_ind + radius + 1 - right_offset,
        ],
        ...
        ].to(out_mask.device),  # added this to move the disk tensor on the same device 
    )
    return out_mask
...
def encode_disks_last_clicks(pcs: List[List[List[Point]]], ncs: List[List[List[Point]]], pos_encoding: List[torch.Tensor], neg_encoding: List[torch.Tensor], radius: int =5) -> Tuple(List[torch.Tensor], List[torch.Tensor]):
    ...
    pos_encoding = disk_mask_from_coords_batch(last_pcs, pos_encoding, radius=radius)
