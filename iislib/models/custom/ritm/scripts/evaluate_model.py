...
from models.custom.ritm.isegm.inference.evaluation import evaluate_dataset
from models.custom.ritm.isegm.inference.predictors import get_predictor
from models.custom.ritm.isegm.utils.exp import load_config_file
from models.custom.ritm.isegm.utils.vis import (
    draw_probmap,
...
def parse_args():
    ...
    group_device.add_argument(
        "--gpus", type=str, default="0", help="ID of used GPU."
    )
    group_device.add_argument(
        "--cpu",
        action="store_true",
    ...
    parser.add_argument(
        "--config-path",
        type=str,
        default="models/custom/ritm/config.yml",
...
def main():
    ...
    (
        ...
        logs_path,
        logs_prefix,
    ) = get_checkpoints_list_and_logs_path(args, cfg)
        ...
        checkpoints_list = [
            Path(
                utils.find_checkpoint(
                    cfg.INTERACTIVE_MODELS_PATH, args.checkpoint
                )
            )
        ]
...
def save_results(
    ...
    iou_thrs = np.arange(
        0.8, min(0.95, args.target_iou) + 0.001, 0.05
    ).tolist()
    ...
    if args.print_ious:
        ...
        mean_ious = np.array([x[:min_num_clicks] for x in all_ious]).mean(
            axis=0
        )
        miou_str = " ".join(
            [
    ...
    log_path = (
        logs_path
        / f"{name_prefix}{args.eval_mode}_{args.mode}_{args.n_clicks}.txt"
    )
    if log_path.exists():
...
def get_prediction_vis_callback(logs_path, dataset_name, prob_thresh):
    ...
    def callback(
        image, gt_mask, pred_probs, sample_id, click_indx, clicks_list
    ):
