...
def parse_args():
    ...
    parser.add_argument(
        "--datasets",
        type=str,
        default="GrabCut,Berkeley,DAVIS,COCO_MVal,SBD",
...
def main():
    ...
    for file in files_list:
        ...
        aggregated_plot_data[data["dataset_name"]][
            data["model_name"]
        ] = np.array(data["all_ious"]).mean(0)
    ...
    for dataset_name, dataset_results in aggregated_plot_data.items():
        ...
        for model_name, model_results in dataset_results.items():
            ...
            miou_str = " ".join(
                [
                    f"mIoU@{click_id}={model_results[click_id-1]:.2%};"
                    for click_id in [1, 3, 5, 10, 20]
                    if click_id <= len(model_results)
                ]
            )
            print(f"{model_name} on {dataset_name}:\n{miou_str}\n")

            plt.plot(
                1 + np.arange(n_clicks),
                model_results,
                linewidth=2,
                label=model_name,
            )
        ...
        plt.title(
            f"mIoU after every click for {dataset_name}", fontsize="x-large"
        )
        plt.grid()
...
def get_files_list(args, cfg):
    if args.folder is not None:
        ...
        for rel_exp_path in args.exp_models:
            ...
            files_list.extend(
                sorted(
                    (exp_path / "plots").glob(checkpoint_prefix + "*.pickle")
                )
            )
