...
def create_annotations(dataset_path, dataset_split="train"):
    ...
    clean_anno_path = (
        _split_path
        / f"{dataset_split}-annotations-object-segmentation_clean.pkl"
    )

    annotations = {
        "image_id_to_masks": defaultdict(
            list
        ),  # mapping from image_id to a list of masks
        "dataset_samples": [],  # list of unique image ids
    }

    with open(
        _split_path / f"{dataset_split}-annotations-object-segmentation.csv",
        "r",
    ) as f:
        ...
        for row in reader:
            ...
            if (_images_path / f"{image_id}.jpg").is_file() and (
                _masks_path / mask_path
            ).is_file():
                annotations["image_id_to_masks"][image_id].append(mask_path)
    annotations["dataset_samples"] = list(
        annotations["image_id_to_masks"].keys()
    )
