...
def load_and_parse_annotations(dataset_path, dataset_split, n_jobs=1):
    dataset_split_folder = (
        "training" if dataset_split == "train" else "validation"
    )

    orig_annotations = loadmat(
        dataset_path / "index_ade20k.mat",
        squeeze_me=True,
        struct_as_record=True,
    )
    image_ids = [
        image_id.split(".")[0]
        for image_id in orig_annotations["index"].item()[0]
        if dataset_split in image_id
    ]
    folders = [
        Path(folder).relative_to("ADE20K_2016_07_26")
        for folder in orig_annotations["index"].item()[1]
        if dataset_split_folder in folder
    ]

    # list of dictionaries with filename and instance to class mapping
    all_layers = parallel_map(
        list(zip(image_ids, folders)),
        worker_annotations_loader,
        n_jobs=n_jobs,
        use_kwargs=False,
        const_args={"dataset_path": dataset_path},
    )
...
def create_annotations(dataset_path, dataset_split="train", n_jobs=1):
    anno_path = (
        dataset_path / f"{dataset_split}-annotations-object-segmentation.pkl"
    )
    image_ids, folders, all_layers = load_and_parse_annotations(
        dataset_path, dataset_split, n_jobs=n_jobs
    )
