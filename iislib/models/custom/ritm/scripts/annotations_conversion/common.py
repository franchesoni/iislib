...
def parallel_map(
    ...
    with ProcessPoolExecutor(max_workers=n_jobs) as pool:
        ...
        if use_kwargs:
            futures = [
                pool.submit(worker, **a, **const_args)
                for a in array[front_num:]
            ]
        else:
            futures = [
                pool.submit(worker, a, **const_args) for a in array[front_num:]
            ]
        tqdm_kwargs = {
            "total": len(futures),
            "unit": "it",
...
def get_masks_hierarchy(masks, masks_meta):
    ...
    def check_inter(i, j):
        ...
        bbox_score = get_bbox_intersection(bbox_i, bbox_j) / get_bbox_area(
            bbox_j
        )
        if bbox_score < 0.7:
            return False
...
def encode_masks(masks):
    ...
    ordered_masks = sorted(list(enumerate(masks)), key=lambda x: x[1].sum())[
        ::-1
    ]
    for global_id, obj_mask in ordered_masks:
        for layer_indx, (layer_mask, layer_objs) in enumerate(
            zip(layers, layers_objs)
        ):
