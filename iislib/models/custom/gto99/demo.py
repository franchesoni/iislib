...
def test(model, args):
    ...
    for i in tqdm.tqdm(range(ious.shape[0])):
        ...
        for j in range(ious.shape[1]):
            trimap, click_region, [y, x], click_cat = robot_click(
                alpha >= 0.5, gt, trimap
            )
            alpha = pred(image, trimap, alpha, model)
            ious[i, j] = jaccard(
                gt == 1, alpha >= 0.5, np.abs(gt - 0.5) < 0.25
            )

            if args.predictions_dir != "":
                cv2.imwrite(
                    f"{args.predictions_dir}/{name}_{i}_{j+1}.png", alpha * 255
                )

    nocs_90 = NOCS(ious, 0.9)
    mIoU = np.mean(ious)
    print(
        f"Average number of clicks to reach 90% {np.mean(nocs_90)} {nocs_90}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weights", default="InterSegSynthFT.pth", help="pytorch state dict"
    )
    ...
    parser.add_argument(
        "--dataset_dir",
        default="/home/franchesoni/adisk/iis_datasets/datasets/GrabCut/",
    ...
    parser.add_argument(
        "--predictions_dir",
        default="",
        help="Where to store predictions, if blank " " dont save ",
    )
    parser.add_argument(
        "--num_clicks", default=20, type=int, help="Number of clicks per image"
    )
