...
def check_input(image, masks, info):
    ...
    ), f"Both image and mask should have three channels instead of \
    {image.ndim} and {masks.ndim}"
    assert (
        min(masks.shape) == masks.shape[2]
    ), f"Mask should be channel last format but its shape is {masks.shape}"
    # works unless dims (of img or number of layers) exceeds normal range
...
def get_all_objs(masks):
    # gets layer index, val for each layer and val and return a list
    return [
...
def check_inclusion(mask1, mask2):
    assert (
        set(np.unique(mask1)) == set(np.unique(mask2)) == {0, 1} == {0, 1}
    ), "Masks are not binary"
...
def random_single(image, masks, info):
    ...
    ]  # select one object randomly (use torch vs np.random for robustness)
    return 1 * (
...
def random_merge(image, masks, info, n_merge=2):
    ...
    return 1 * np.logical_or(
        [masks[:, :, obj["layer_ind"]] == obj["val"] for obj in sel_objs]
    )
